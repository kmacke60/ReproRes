...
#widestep <- subset(widestep, select=-c(date))
...
#step_nomiss <-  transform(step_nomiss, intervalf=factor(interval))
intmean2 <- aggregate(step_nomiss, by = list(step_nomiss$interval, step_nomiss$weekday), mean)
intmean2$day <- rep(c("Weekend", "Weekday"), each=288)
#intmean2 <- aggregate(step ~ intday, data = step_nomiss, mean)
maxstep = max(intmean2$step)
print(maxstep)
xyplot(step ~ interval | day, data = intmean2, layout = c(1,2), type="l",ylab="Number of steps")
#max_index <- which(intmean2$interval == max(intmean2$step))
#p <- ggplot(intmean2, aes(interval, step,group=day))
#p + geom_line(aes(colour=day)) + labs(x = "5 minute interval", 
#                y = "Mean step count", title = "Activity Pattern")
#max_interval <- step_nomiss$interval[max_index]
#max_interval
#temp <- as.data.frame(intmean2)
#intn <- unique(step_nomiss$interval)
#temp$interval <- intn
#plot(interval, intmean2, type="l", xlab="5 minute interval", ylab = "Average number of steps")
